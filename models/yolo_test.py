...
from models.model_detection_heads import Detect, LastFrameDetect, MidFrameDetect, FullFramesDetect, ThermalRgbDetect, LastFrameThermalRgbDetect

class Model(nn.Module):

    def __init__(self, cfg='yolov5s.yaml', ch=3, numframes=1, nc=None, anchors=None, use_tadaconv=False):  # model, input channels, number of classes
        ...
        if use_tadaconv:
            self.model, self.save = parse_model_tadaconv(deepcopy(self.yaml), ch=[ch], nf=numframes) 
        else:
            self.model, self.save = parse_model(deepcopy(self.yaml), ch=[ch], nf=numframes)  # model, savelist
        # print(self.model)
        self.names = [str(i) for i in range(self.yaml['nc'])]  # default names
        # logger.info([x.shape for x in self.forward(torch.zeros(1, ch, 64, 64))])

        # Build strides, anchors
        if 'detectorIDs' in self.yaml.keys():
            ms = [self.model[i] for i in self.yaml['detectorIDs']]
        else:
    ...
    def forward_once(self, x, x2, profile=False):
        ...
        for m in self.model:
            ...
            if profile:
                x_profile = x if m.f != -4 else x2
                o = thop.profile(m, inputs=(x_profile,), verbose=False)[0] / 1E9 * 2 if thop else 0  # FLOPS
                t = time_synchronized()
                for _ in range(100):
                    _ = m(x_profile)
                dt.append((time_synchronized() - t))
                para = m.np / 1000. / 1000.
                if m == self.model[0]:
                    logger.info(f"{'time (ms)':>10s} {'GFLOPS':>10s} {'params':>10s}  {'module'}")
                logger.info(f'{dt[-1]:10.5f} {o:10.2f} {para:10.3f}  {m.type}')
    ...
    def _initialize_biases(self, m, cf=None):  # initialize biases into Detect(), cf is class frequency
        ...
        if isinstance(m, ThermalRgbDetect):
...
def parse_model(d, ch, nf=1):  # model_dict, input_channels(3)
    ...
    # print("ch", ch)
    layer_list = d['backbone'] + ((d['headRGB'] + d['headThermal']) if 'detectorIDs' in d.keys() else []) + d['head']
        ...
        if m in [Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, DWConv, MixConv2d, Focus, CrossConv, BottleneckCSP,
            ...
            if m is Focus:
                ...
                if m in [BottleneckCSP, C3, C3TR]:
                    ...
                    n = 1
        elif m in [DeformableSpatialAttention,
            ...
            c2 = ch[f[0]]
            args = [c2]
        elif m in [Detect, LastFrameDetect, MidFrameDetect, FullFramesDetect]:
            args.append([ch[x] for x in f])
            if isinstance(args[1], int):  # number of anchors
                args[1] = [list(range(args[1] * 2))] * len(f)
        elif m in [LastFrameThermalRgbDetect]:
            args = [*args]
            # args.append([ch[x] for x in f])
            if isinstance(args[1], int):  # number of anchors
                args[1] = [list(range(args[1] * 2))] * len(f)
...
def parse_model_tadaconv(d, ch, nf=1):  # model_dict, input_channels(3)
    logger.info('\n%3s%18s%3s%10s  %-40s%-30s' % ('', 'from', 'n', 'params', 'module', 'arguments'))
    anchors, nc, gd, gw = d['anchors'], d['nc'], d['depth_multiple'], d['width_multiple']
    na = (len(anchors[0]) // 2) if isinstance(anchors, list) else anchors  # number of anchors
    no = na * (nc + 5)  # number of outputs = anchors * (classes + 5)
