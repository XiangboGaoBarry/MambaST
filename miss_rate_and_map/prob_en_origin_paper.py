...
def apply_late_fusion(det_1, det_2, method, det_3=''):

    jdict = []

    for i in tqdm(range(len(det_2['image']))):
        info_1 = {}
        info_1['img_name'] = det_1['image'][i]
        info_1['bbox'] = det_1['boxes'][i]
        info_1['score'] = det_1['scores'][i]
        ...
        for j in range(len(out_boxes)):
            jdict.append({'image_id': i, 
                    'category_id': 1.0, 
                    'bbox': [round(box,7) for box in out_boxes[j].tolist()],
                    'score': round(out_scores[j].item(),5)})
    
    return jdict

if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    # parser.add_argument('--prediction_path', type=str, default='prediction_folder_prob_en', help='prediction path')
    parser.add_argument('--det_file_1', type=str, 
                        default='./runs/test/yolov5l_fusion_transformerx3_Kaist_aligned_tadaconv_mlpSTmixv2_same_num_patches/best_predictions_ct2_prob_en.json', 
                        help='model.json path')
    parser.add_argument('--det_file_2', type=str, 
                        default='./runs/test/yolov5l_fusion_transformerx3_Kaist_aligned_tadaconv_DSATv1_lframe3_stride3/best_predictions_ct2_prob_en.json', 
                        help='model.json path')
    parser.add_argument('--det_file_3', type=str, 
                        default='./runs/test/yolov5l_fusion_transformerx3_Kaist_aligned_tadaconv_DSA/best_predictions_ct2_prob_en.json', 
                        help='model.json path')
    ...
    parser.add_argument('--box_fusion', type=str, default='argmax', help='model.json path')
    args = parser.parse_args()
    # prediction_folder = args.prediction_path
    out_folder = args.outfolder
    
    det_file_1 = args.det_file_1
    det_file_2 = args.det_file_2
    det_file_3 = args.det_file_3
    ...
    # 3 inputs
    result = apply_late_fusion(det_1, det_2, method, det_3=det_3)
    with open(out_folder + '/out1.json', 'w') as f:
        json.dump(result, f)
    # 2 inputs only
