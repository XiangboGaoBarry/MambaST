...
Modified from  https://github.com/sejong-rcv/MLPD-Multi-Label-Pedestrian-Detection/blob/main/evaluation_script/evaluation_script.py
"""
...
class KAISTPedEval(COCOeval):
    ...
    def _prepare(self, id_setup):
        ...
        # set ignore flag
        for gt in gts:
            # TODO: Bug: for sanitized data, ignore flag is set to be 1 manually
            # gt['ignore'] = 0
...
def evaluate(test_annotation_file: str, user_submission_file: str or list, phase_codename: str = 'Multispectral'):
...
if __name__ == "__main__":
    ...
    phase = "Multispectral"
    # rstFile = open("/mnt/roahm/users/akanu/projects/fusion_therm_rgb/state-of-art-detectors/yolov5/runs/test/yolov5l_ir_from_pretrained_conf_thres_2_/best_predictions.json")
    # rstFile = "/mnt/roahm/users/akanu/projects/fusion_therm_rgb/state-of-art-detectors/yolov5/runs/test/yolov5l_ir_from_pretrained_conf_thres_2_/best_predictions.json"
    # rstFile = json.load(rstFile)
    # results = [evaluate(args.annFile, rstFile, phase)]
    
    rstFile = args.rstFiles[0]
    ...
    if args.multiple_outputs:
        ...
        for i in range(0,100):
            # temp = f'cur_{i}_predictions_ct2.json'
            temp = f'cur_{i}_predictions_ct001_conf_iou0.5_merge.json'
            rstPath = spath + '/' + temp
            if os.path.exists(rstPath):
                rstFile_list.append(rstPath)
        args.rstFiles = rstFile_list
    ...
    for result in results:
        json_dict['all'].append(result['all'].summarize(0))
        json_dict['day'].append(result['day'].summarize(0))
        json_dict['night'].append(result['night'].summarize(0))
        json_dict['recall_all'].append(1 - result['all'].eval['yy'][0][-1])
        
    if args.multiple_outputs:
        save_loc = str(path.parent) + '/' + 'missrate.json'
    else:
        save_loc = str(path.parent) + '/' + f'missrate_{path.name}' #path.name has .json included
